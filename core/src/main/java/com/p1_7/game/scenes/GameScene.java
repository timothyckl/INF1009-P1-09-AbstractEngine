...
public class GameScene extends Scene implements GamePhaseListener {
    ...
    /**
     * Colour used to draw wall calibration outlines.
     * Cyan at 60% alpha so the background art shows through while walls are clearly visible.
     * Once MazeLayout numbers match the background image, set alpha to 0f or empty the
     * render body to hide them without touching collision logic.
     */
    private static final Color WALL_DEBUG_COLOUR = new Color(0f, 1f, 1f, 0.6f);
    ...
    private IRenderable createWallRenderable(float[] rect) {
        ...
        return new IRenderable() {
            ...
            public void render(IDrawContext ctx) {
                // CALIBRATION: draws a cyan outline over each collision wall so you can see
                // exactly where the engine walls sit vs the dungeon background image.
                // Tweak MazeLayout constants (LAYOUT_SCALE, LAYOUT_OFFSET_Y, hall centres, etc.)
                // until these boxes line up with the image walls, then either:
                //   - Set WALL_DEBUG_COLOUR alpha to 0f to make them invisible, OR
                //   - Replace this call with `filled=true` and WALL_FILL_COLOUR for opaque walls.
                ((GdxDrawContext) ctx).rect(
                    WALL_DEBUG_COLOUR,
                    stableRect[0], stableRect[1], stableRect[2], stableRect[3], false);
            }
        };
    }
    ...
    /** asset path for the dungeon background image used during gameplay */
    private static final String GAME_BG_ASSET = "game-background.png";

    private IRenderable createBackgroundRenderable() {
        // Capture dimensions once at scene entry. The background image is stretched to
        // fill the entire playfield so it always covers the screen regardless of the
        // window size chosen by the player. Wall collision geometry (WallCollidable) is
        // defined in fixed logical coordinates and is completely independent of this
        // visual, so resizing the window never affects collision behaviour.
        final float bgW = Settings.getWindowWidth();
        final float bgH = HudStrip.PLAYFIELD_HEIGHT;
        final Transform2D transform = new Transform2D(0f, 0f, bgW, bgH);
        return new IRenderable() {
            @Override public String getAssetPath() { return GAME_BG_ASSET; }
            @Override public ITransform getTransform() { return transform; }

            @Override
            public void render(IDrawContext ctx) {
                ((GdxDrawContext) ctx).drawTexture(GAME_BG_ASSET, 0f, 0f, bgW, bgH);
            }
        };
    }
